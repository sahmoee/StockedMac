...
import { handleContentRecipes, handleContentImage, handleAdminCatalogChunk, contentOriginFor } from "./src/content.js";
import { handleHarvestCache, handleHarvestImage, handleHarvestRecipes, handleHarvestImageGet } from "./src/harvest.js";
...
import { modelsForTier } from "../../shared/models.js";
...
// Per-route model tier. Unmapped routes default to "balanced".
//   strong   — Opus/Fable: complex multi-step reasoning (meal planning)
//   balanced — Sonnet: standard AI tasks (receipts, recipes, inventory)
//   fast     — Haiku: quick parsing, lookup, schema cleanup
const ROUTE_TIER = Object.freeze({
  mealPlanOptimize:    "strong",
  barcode:             "fast",
  recipeEnrich:        "fast",
  recipeFix:           "fast",
  recipeSubstitutions: "fast",
  receiptReview:       "fast",
  inventoryNormalize:  "fast",
  inventoryScan:       "fast",   // Haiku handles structured formatting well; ~5-10 s vs 20-30 s for Sonnet
});

// Per-route Anthropic call timeout (ms). Two calls (initial + repair) must fit inside the
// iOS URLSession timeout (90 s). Default is ai.js DEFAULT_TIMEOUT_MS = 25 000 ms.
const ROUTE_TIMEOUT = Object.freeze({
  inventoryScan:    20000,  // 20 s × 2 passes = 40 s < 90 s iOS timeout
  inventoryIntent:  20000,
  receiptText:      20000,
  receiptImage:     20000,
});
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // ── Admin: bulk recipe catalog upload (POST; shared-key gated, Mac admin only) ──
      if (request.method === "POST" && url.pathname === "/admin/catalog/chunk") {
        route = "adminCatalogChunk";
        return done(await handleAdminCatalogChunk(request, env, requestId));
      }

      // ── Harvest cache (Build 91 of the Mac app): recipes + images pushed by the
      //     Mac's Browse section, readable by every device on the key. ──
      if (url.pathname === "/harvest/cache" && request.method === "POST") {
        route = "harvestCache";
        if (await isLimited(limiterFor(env, "default"), rlKey, "harv")) {
          status = 429; return errJson(429, "Rate limit exceeded", { code: "rateLimited", requestId });
        }
        return done(await handleHarvestCache(request, env, ctx, requestId));
      }
      if (url.pathname === "/harvest/image" && request.method === "POST") {
        route = "harvestImage";
        if (await isLimited(limiterFor(env, "default"), rlKey, "harv")) {
          status = 429; return errJson(429, "Rate limit exceeded", { code: "rateLimited", requestId });
        }
        return done(await handleHarvestImage(request, env, ctx, requestId));
      }
      if (url.pathname === "/harvest/recipes" && request.method === "GET") {
        route = "harvestRecipes";
        return done(await handleHarvestRecipes(request, env, ctx, requestId));
      }
      if (url.pathname.startsWith("/harvest/img/") && request.method === "GET") {
        route = "harvestImg";
        return done(await handleHarvestImageGet(request, env, ctx, requestId, url.pathname));
      }
      ...
      // cfg.aiModel pins a single model for the entire deployment; otherwise
      // the tier system picks the best current model for the route's complexity.
      const cfg = await getRuntimeConfig(env);
      const cfgModel = resolveModel(cfg, null, null);
      const models = cfgModel
        ? [cfgModel]
        : await modelsForTier(ROUTE_TIER[prompt.route] || "balanced", env);
      const maxTokens = resolveMaxTokens(cfg, prompt.route, prompt.maxTokens);
      const timeoutMs = ROUTE_TIMEOUT[prompt.route];   // undefined → ai.js default (25 000 ms)
      ...
      if (prompt.validated) {
        const r = await runValidatedRoute({
          ...
          system: prompt.system, user: prompt.user, maxTokens, timeoutMs, promptCache: true, requestId,
        });
        if (!r.ok) {
          status = 502;
      ...
      const res = await callAnthropicResilient({ env, models, system: prompt.system, user: prompt.user, maxTokens, timeoutMs, promptCache: true, requestId });
